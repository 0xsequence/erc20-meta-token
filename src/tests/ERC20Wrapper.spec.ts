...
contract('ERC20Wrapper', (accounts: string[]) => {
  ...
  // Contracts
  let ownerErc20WrapperContract: Erc20Wrapper
  let userErc20WrapperContract: Erc20Wrapper
  ...
  context('When Erc20Wrapper contract is deployed', () => {
    ...
    beforeEach(async () => {
      // Deploy Erc20Wrapper
      let abstractErc20Wrapper = await AbstractContract.fromArtifactName('ERC20Wrapper')
      ownerErc20WrapperContract = await abstractErc20Wrapper.deploy(ownerWallet) as Erc20Wrapper
      userErc20WrapperContract = await ownerErc20WrapperContract.connect(userSigner) as Erc20Wrapper
      ...
      wrapperAddress = ownerErc20WrapperContract.address
    })

    describe('getter functions', () => {
      describe('supportsInterface()', () => {
        it('should return true for 0x01ffc9a7 (IERC165)', async () => {
          const support = await ownerErc20WrapperContract.functions.supportsInterface('0x01ffc9a7')
          expect(support[0]).to.be.eql(true)
        })
  
        it('should return true for 0x4e2312e0 (IERC1155Receiver)', async () => {
          const support = await ownerErc20WrapperContract.functions.supportsInterface('0x4e2312e0')
          expect(support[0]).to.be.eql(true)
        })

        it('should return true for 0xd9b67a26 (IERC1155)', async () => {
          const support = await ownerErc20WrapperContract.functions.supportsInterface('0xd9b67a26')
          expect(support[0]).to.be.eql(true)
        })
      })
    })
    ...
    describe('deposit() function', () => {
      ...
      describe('when depositing tokens', () => {
        ...
        it('should REVERT if user did not approve wrapper contract', async () => {
          const tx = userErc20WrapperContract.functions.deposit(tokenAddress, userAddress, depositAmount, txParam)
        ...
        context('When token contract is approved', () => {
          ...
          it('should REVERT if user does not have sufficient funds', async () => {
            const tx = userErc20WrapperContract.functions.deposit(tokenAddress, userAddress, INIT_BALANCE + 1, txParam)
            await expect(tx).to.be.rejected;
          })

          it('should REVERT if msg.value is not 0', async () => {
            const tx = userErc20WrapperContract.functions.deposit(tokenAddress, userAddress, depositAmount, {gasLimit: 1000000, value: 1})
            await expect(tx).to.be.rejectedWith( RevertError("ERC20Wrapper#deposit: NON_NULL_MSG_VALUE") )
          })

          it('should REVERT if recipient is 0x0', async () => {
            const tx = userErc20WrapperContract.functions.deposit(tokenAddress, ZERO_ADDRESS, depositAmount, txParam)
            await expect(tx).to.be.rejectedWith(RevertError("ERC20Wrapper#deposit: INVALID_RECIPIENT"));
          })
          ...
          it('should PASS if recipient is different from sender', async () => {
            const tx = userErc20WrapperContract.functions.deposit(tokenAddress, receiverAddress, depositAmount, txParam)
            await expect(tx).to.be.fulfilled;
          })
          ...
          context('When tokens are deposited', () => {
            let tx;
            beforeEach(async () => {
              tx = await userErc20WrapperContract.functions.deposit(tokenAddress, userAddress, depositAmount, txParam)
            })

            it('should increase nTokens value by 1', async () => {
              const nTokens = await userErc20WrapperContract.functions.getNTokens()
              expect(nTokens[0]).to.be.eql(BigNumber.from(2))
            })

            it('should set id of token to 2', async () => {
              const id = await userErc20WrapperContract.functions.getTokenID(tokenAddress)
            ...
            it('should increase ERC1155 balance of user by the right amount for given token', async () => {
              const balance_1 = await userErc20WrapperContract.functions.balanceOf(userAddress, 2)
              expect(balance_1[0]).to.be.eql(depositAmount)

              await userErc20WrapperContract.functions.deposit(tokenAddress, receiverAddress, depositAmount, txParam)
  
              const balance_2 = await userErc20WrapperContract.functions.balanceOf(receiverAddress, 2)
            ...
            it('should NOT emit a TokenRegistration events for already registered token', async () => {
              let tx2 = await userErc20WrapperContract.functions.deposit(tokenAddress, userAddress, depositAmount, txParam)
      ...
      describe('when depositing ETH', () => {
        ...
        it('should REVERT if msg.value is not equal to value', async () => {
          ...
          let tx = userErc20WrapperContract.functions.deposit(ETH_ADDRESS, userAddress, depositAmount, 
            {gasLimit:1000000, value: depositAmount.sub(1)}
          )
          await expect(tx).to.be.rejectedWith( RevertError("ERC20Wrapper#deposit: INCORRECT_MSG_VALUE") )
          
          // Msg.value is larger than value
          tx = userErc20WrapperContract.functions.deposit(ETH_ADDRESS, userAddress, depositAmount, 
        ...
        it('should REVERT if recipient is 0x0', async () => {
          const tx = userErc20WrapperContract.functions.deposit(ETH_ADDRESS, ZERO_ADDRESS, depositAmount, 
        ...
        it('should PASS if recipient is different from sender', async () => {
          ...
          //msg.value is smaller than value
          let tx = userErc20WrapperContract.functions.deposit(ETH_ADDRESS, receiverAddress, depositAmount, 
            {gasLimit:1000000, value: depositAmount}
          )
          await expect(tx).to.be.fulfilled
        })
        ...
        context('When ETH were deposited', () => {
          let tx
          beforeEach(async () => {
            tx = await userErc20WrapperContract.functions.deposit(ETH_ADDRESS, userAddress, depositAmount,
              {gasLimit:1000000, value: depositAmount}
            )
          })
          ...
          it('should increase ERC1155 balance of user by the right amount for given token', async () => {
            const balance_1 = await userErc20WrapperContract.functions.balanceOf(userAddress, ETH_ADDRESS)
            expect(balance_1[0]).to.be.eql(depositAmount)

            await userErc20WrapperContract.functions.deposit(ETH_ADDRESS, receiverAddress, depositAmount,
              {gasLimit:1000000, value: depositAmount}
            )

            const balance_2 = await userErc20WrapperContract.functions.balanceOf(receiverAddress, ETH_ADDRESS)
            expect(balance_2[0]).to.be.eql(depositAmount)
          })
    ...
    describe('fallback function', () => {
      ...
      context('When ETH were deposited', () => {
        ...
        it('should increase ERC1155 balance of user by the right amount for given token', async () => {
          const balance = await userErc20WrapperContract.functions.balanceOf(userAddress, ETH_ADDRESS)
          expect(balance[0]).to.be.eql(depositAmount)
        })
    ...
    describe('withdraw function', () => {
      ...
      describe('when withdrawing tokens', () => {
        ...
        beforeEach(async () => {
          ...
          await userErc20WrapperContract.functions.deposit(tokenAddress, userAddress, depositAmount) // Deposit tokens
        })
        ...
        it('should REVERT if token is not registered', async () => {
          const tx = userErc20WrapperContract.functions.withdraw(userAddress, userAddress, depositAmount, txParam)
          await expect(tx).to.be.rejectedWith(RevertError('ERC20Wrapper#getTokenID: UNREGISTERED_TOKEN'))
        })

        it('should REVERT if recipient is 0x0', async () => {
          const tx = userErc20WrapperContract.functions.withdraw(tokenAddress, ZERO_ADDRESS, depositAmount, txParam)
          await expect(tx).to.be.rejected;
        })

        it('should PASS if user has sufficient wrapped tokens', async () => {
          const tx = userErc20WrapperContract.functions.withdraw(tokenAddress, userAddress, depositAmount, txParam)
          await expect(tx).to.be.fulfilled
        })

        it('should PASS when withdrawing to another address', async () => {
          const tx = userErc20WrapperContract.functions.withdraw(tokenAddress, receiverAddress, depositAmount, txParam)
          await expect(tx).to.be.fulfilled
        })

        context('When tokens are withdrawn', () => {
          beforeEach(async () => {
            await userErc20WrapperContract.functions.withdraw(tokenAddress, userAddress, depositAmount, txParam)
          })
          ...
          it('should decrease ERC1155 balance of user by the right amount for given token', async () => {
            const balance = await userErc20WrapperContract.functions.balanceOf(userAddress, tokenAddress)
            expect(balance[0]).to.be.eql(depositAmount.sub(withdrawAmount))
          })
      ...
      describe('when withdrawing ETH', () => {
        ...
        beforeEach(async () => {
          // Depositing ETH
          await userErc20WrapperContract.functions.deposit(ETH_ADDRESS, userAddress, depositAmount, 
            {gasLimit:1000000, value: depositAmount}
          )
        })
        ...
        it('should REVERT if recipient is 0x0', async () => {
          const tx = userErc20WrapperContract.functions.withdraw(ETH_ADDRESS, ZERO_ADDRESS, depositAmount, txParam)
          await expect(tx).to.be.rejectedWith( RevertError("ERC20Wrapper#withdraw: INVALID_RECIPIENT") )
        })

        it('should PASS if user has sufficient wrapped tokens', async () => {
          const tx = userErc20WrapperContract.functions.withdraw(ETH_ADDRESS, userAddress, depositAmount, txParam)
          await expect(tx).to.be.fulfilled
        })

        it('should PASS when withdrawing to another address', async () => {
          const tx = userErc20WrapperContract.functions.withdraw(ETH_ADDRESS, receiverAddress, depositAmount, txParam)
          await expect(tx).to.be.fulfilled
        })
        ...
        context('When ETH are withdrawn', () => {
          ...
          beforeEach(async () => {
            ...
            await userErc20WrapperContract.functions.withdraw(ETH_ADDRESS, receiverAddress, depositAmount, txParam)
          })
    ...
    describe('onERC1155Received function', () => {
      ...
      describe('when withdrawing tokens', () => {
        ...
        it('should REVERT if token is not registered', async () => {
          //@ts-ignore
          const tx = userErc20WrapperContract.functions.safeTransferFrom(userAddress, wrapperAddress, 666, 0, data)
          await expect(tx).to.be.rejectedWith(RevertError('ERC20Wrapper#getIdAddress: UNREGISTERED_TOKEN'))
        })

        it('should PASS if user has sufficient wrapped tokens', async () => {
          //@ts-ignore
          const tx = userErc20WrapperContract.functions.safeTransferFrom(userAddress, wrapperAddress, tokenID, depositAmount, data)
          await expect(tx).to.be.fulfilled
        })
        ...
        context('When tokens are withdrawn', () => {
          beforeEach(async () => {
            //@ts-ignore
            await userErc20WrapperContract.functions.safeTransferFrom(userAddress, wrapperAddress, tokenID, depositAmount, data)
          })
      ...
      describe('when withdrawing ETH', () => {
        ...
        it('should PASS if user has sufficient wrapped tokens', async () => {
          //@ts-ignore
          const tx = userErc20WrapperContract.functions.safeTransferFrom(userAddress, wrapperAddress, ONE_ID, depositAmount, data)
          await expect(tx).to.be.fulfilled
        })
        ...
        context('When ETH are withdrawn', () => {
          ...
          beforeEach(async () => {
            receiverPreBalance = await provider.getBalance(receiverAddress)
            //@ts-ignore
            await userErc20WrapperContract.functions.safeTransferFrom(userAddress, wrapperAddress, ONE_ID, depositAmount, data) 
          })
      ...
      describe('when withdrawing tokens', () => {
        beforeEach(async () => {
          for (let i = 0; i < noTokens; i++) {
            ...
            await userErc20WrapperContract.functions.deposit(tokenContracts[i].address, userAddress, depositAmount) // Deposit tokens
          }
        })
        ...
        it('should PASS if user has sufficient wrapped tokens', async () => {
          //@ts-ignore
          const tx = userErc20WrapperContract.functions.safeBatchTransferFrom(userAddress, wrapperAddress, tokenIDs, depositAmounts, data)
          await expect(tx).to.be.fulfilled
        })
        ...
        context('When tokens are withdrawn', () => {
          beforeEach(async () => {
            //@ts-ignore
            await userErc20WrapperContract.functions.safeBatchTransferFrom(userAddress, wrapperAddress, tokenIDs, depositAmounts, data)
          })
          ...
          it('should decrease ERC1155 balance of user by the right amount for given token', async () => {
            for (let i = 0; i < noTokens; i++) {
              const balance = await userErc20WrapperContract.functions.balanceOf(userAddress, tokenContracts[i].address)
              expect(balance[0]).to.be.eql(depositAmount.sub(withdrawAmount))
            }
          })
